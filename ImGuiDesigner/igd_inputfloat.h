...
namespace igd
{
	...
	public:
		...
		std::string ScriptInternal() {
			...
			code << "ImGui::InputFloat(\"" << v_label << "##" << v_id << "\", &" << v_variable_name << ", " << igd::script::GetFloatString (step) << ", " << igd::script::GetFloatString (step_fast) << ", \"" << format << "\", " << igd::script::BuildFlagString(this) << ");";
			return code.str();
		};
