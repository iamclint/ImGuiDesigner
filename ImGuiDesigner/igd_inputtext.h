	...
	public:
		...
		virtual std::string RenderInternal() override
		{
			...
			if (v_size.type == Vec2Type::Absolute && v_size.value.x != 0)
			{
				...
				code << "ImGui::SetNextItemWidth(" << igd::fString(v_size.value.x) << ");";
			}
			else if (v_size.type == Vec2Type::Relative && v_size.value.x != 0)
			{
				ImGui::SetNextItemWidth(ContentRegionAvail.x * (v_size.value.x / 100));
				code << "ImGui::SetNextItemWidth(" << igd::fString(ContentRegionAvail.x * (v_size.value.x / 100)) << ");";
			}
			ImGui::InputText((v_label + "##" + v_id).c_str(), &input_data, v_flags);
			code << "ImGui::InputText(\"" << v_label << "##" << v_id << "\", &" << v_id << ", " << this->buildFlagString() << ");";
			return code.str();
		}
